...
- Device card overlay buttons laid out as a 2×2 grid: row 1 `[configure stream][connect]`, row 2 `[shell][list files]`. `.services` swapped from `flex-wrap` to `display: grid; grid-template-columns: 1fr 1fr; place-items: center`; the `.services-break` hack div is gone. Connect button created inline after configure stream (was appended last); `.services-label` spans both columns via `grid-column: 1 / -1`
