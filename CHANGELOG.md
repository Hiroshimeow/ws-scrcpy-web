...
- **Final "Stopping X" log lines reach the console reliably on Ctrl+C.** Follow-up to the readline-removal below — once shutdown completed in <50ms, the trailing log lines (`Stopping HTTP(s) Server Service...`, `Stopping WebSocket Server Service...`, `Stopping Connected Devices...`, `[WebSocket Server] stopped`) were being dropped. The reader saw only `Received signal SIGINT` then nothing. Root cause: dev-supervisor uses `stdio: 'inherit'`, so the child writes directly to PowerShell's stdout FD, and per Node docs `process.stdout` writes to a TTY are async on Windows — the writes were queued in Node's stream buffer and the process exited before the buffer flushed to the console. (`Logger.writeToFile` uses `fs.appendFileSync` so the on-disk log always had every line — the bug was console presentation only.) An initial attempt to drain via `process.stdout.write('', cb)` failed because Node short-circuits empty writes (cb fires on nextTick without waiting for prior queued writes). Fixed by calling `process.stdout._handle.setBlocking(true)` + same for stderr at the top of `exit()` — forces both streams to synchronous mode, so every subsequent `console.log` returns only after the bytes hit the TTY. Internal API but well-established Node pattern; wrapped in try/catch so degradation on a future Node is to today's behavior, not a crash. The 10s watchdog stays as belt-and-suspenders for any future pin.
