...
- **Final "Stopping X" log lines reach the console reliably on Ctrl+C.** Follow-up to the readline-removal below — once shutdown completed in <50ms, the trailing log lines (`Stopping HTTP(s) Server Service...`, `Stopping WebSocket Server Service...`, `Stopping Connected Devices...`, `[WebSocket Server] stopped`) were being dropped. The reader saw only `Received signal SIGINT` then nothing. Root cause: dev-supervisor uses `stdio: 'inherit'`, so the child writes directly to PowerShell's stdout FD, and per Node docs `process.stdout` writes to a TTY are async on Windows — the writes were queued in Node's stream buffer and the process exited before the buffer flushed to the console. Fixed deterministically by `process.stdout.write('', cb)` + `process.stderr.write('', cb)` chained, then `process.exit(0)` — `write('', cb)` resolves only when the stream's internal write queue empties, so we exit exactly as fast as the buffer actually drains (no hard-coded delay). The 10s watchdog stays as belt-and-suspenders for the edge case where the cb chain never fires (stream destroyed mid-write, OS handle issue, etc.).
