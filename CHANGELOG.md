...
- **Deliberate 500ms hold before final event-loop drain on Ctrl+C** so PowerShell's prompt-redraw doesn't interleave with the shutdown log output. Real-shutdown now finishes in 10s of ms, which is faster than `npm.cmd`'s Ctrl+C acknowledgment to PowerShell — PS would redraw its prompt mid-output. A ref'd 500ms `setTimeout` no-op in `exit()` holds the loop alive long enough for the prompt-redraw to settle on top of completed output. No correctness cost — the 10s watchdog backstops any real hang. Per user direction.
