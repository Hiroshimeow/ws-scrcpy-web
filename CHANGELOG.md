...
- **Final "Stopping X" log lines reach the console reliably on Ctrl+C.** Follow-up to the readline-removal below — once shutdown completed in <50ms, the trailing log lines (`Stopping HTTP(s) Server Service...`, `Stopping WebSocket Server Service...`, `Stopping Connected Devices...`, `[WebSocket Server] stopped`) were being dropped because the child process exited before the dev-supervisor's pipe-read had drained the OS pipe buffer. The reader saw only `Received signal SIGINT` then nothing. Fixed by adding a ref'd 250ms `setTimeout` no-op in `exit()` — the child's event loop stays alive just long enough for the pipe to flush through the supervisor to PowerShell, then drains and exits naturally. Verified by user with three repro runs (empty trailing output before fix, full sequence visible after).
