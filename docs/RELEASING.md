...
6. **Commit and tag.**
   ...
   ```
7. **Watch CI.** The tag push triggers `.github/workflows/release.yml`. Both Windows and Linux jobs run in parallel; the `publish` job uploads artifacts (Setup.exe, MSI, portable ZIP, AppImage, AppImage.sig once SignPath is live, nupkg, releases.stable.json, SHA256SUMS) and creates a GitHub Release with auto-generated notes. The MSI is generated by `vpk pack --msiDeploymentTool` and requires WiX Toolset on the runner — installed via `choco install wixtoolset`.
   ```bash
   gh run watch
   ```
...
2. **Add the four non-secret identifiers** as repository variables (NOT secrets -- they're visible in the workflow runs):
   ...
   - `SIGNPATH_WINDOWS_POLICY_SLUG` -- the slug for the policy that signs Windows EXEs, Setup.exe, and the MSI (one Authenticode policy covers all)
...
4. **Verify the release.** The `mode=signed` branch in `.github/workflows/release.yml` will fire automatically once `SIGNPATH_API_TOKEN` exists. Confirm:
   - The Setup.exe, MSI, launcher.exe, and tray.exe are signed by SignPath Foundation (Properties → Digital Signatures on Windows).
